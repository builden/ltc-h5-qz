...
function makeSig(method, cmd, params) {
  ...
  return util.uriEncode(util.hmac(APPKEY + '&', rst));
}
...
function ctorUrl(cmd, params) {
  ...
  return TX_OPEN_SERVER + cmd + '?' + p + 'sig=' + makeSig('get', cmd, params);
}
...
/*
 *  要求: 除了0~9 a~z A~Z !*() 之外,其他字符按其ASSCII码十六进制加%进行表示.
 *  encodeURIComponent()函数作用: 除了0~9 a~z A~Z以及-_.!~*'() 之外
 */
...
exports.checkSig = function (pathname, query) {
  var params = [];
  var originSig = query.sig;
  Object.keys(query).forEach((key) => {
    if (key !== 'sig') params.push([key, decodeURI(String(query[key]))]);
  });
  params = params.sort();

  var sig = makeSig('get', pathname, params);
  return (sig === originSig);
}
